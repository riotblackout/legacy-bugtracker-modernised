...
select u.us_id, u.us_username, u.us_org, u.us_bugs_per_page, u.us_enable_bug_list_popups,
       ...
       u.us_admin,
       (select count(*)
          from project_user_xref px
         where px.pu_user = u.us_id
           and px.pu_admin = 1) as project_admin
from users u
inner join orgs org 
...
where us_username = @us and u.us_active = 1");
            ...
            else
            {
                ...
                if (dr.Table.Columns.Contains("project_admin") && dr["project_admin"] != DBNull.Value)
                {
                    projectAdmin = Convert.ToInt32(dr["project_admin"]);
                }
